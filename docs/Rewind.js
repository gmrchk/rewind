...
var _Rewind = __webpack_require__(2);

module.exports = _Rewind.Rewind;
...
var _Dragger = __webpack_require__(3);
 ...
 */
var Rewind = exports.Rewind = function Rewind(element, options) {
    var _this = this;

    _classCallCheck(this, Rewind);

    this.onDragStart = function () {
        _this.startVolume = parseInt(_this.element.volume * 100);
        _this.startTime = parseInt(_this.element.currentTime);
    };

    this.onDragEnd = function (event) {
        if (_this.dragger.currentlyAdjusting === 'x') {
            _this.element.currentTime = _this.currentTime;
            _this.options.onTimeChange({ time: _this.currentTime });
        }

        setTimeout(function () {
            return _this.isAdjusting = false;
        });
        _this.currentTime = null;
        _this.startVolume = null;
        _this.startTime = null;
    };

    this.onDrag = function (distance, absoluteDistance, direction, event) {
        _this.isAdjusting = true;

        if (direction === "x") {
            var currentTime = null;
            var time = parseInt(distance * _this.options.timeSpeed + _this.startTime);

            if (time < 0) {
                currentTime = 0;
            } else if (time > _this.element.duration) {
                currentTime = _this.element.duration;
            } else {
                currentTime = time;
            }

            _this.currentTime = currentTime;
            _this.options.onPreviewTimeChange({ time: _this.currentTime });

            return (0, _utils.toMMSS)(currentTime);
        } else {
            var volume = null;
            var volumeToSet = parseInt(_this.startVolume - distance * _this.options.volumeSpeed / 2);
            ...
            _this.element.volume = volume / 100;
            _this.options.onVolumeChange({ volume: volume });
    ...
    var defaults = {
        ...
        onPreviewTimeChange: function onPreviewTimeChange() {},
    ...
    // dragger instance
    this.dragger = new _Dragger.Dragger(_extends({}, this.options, {
        element: element,
        onDragStart: this.onDragStart,
        onDragEnd: this.onDragEnd,
        onDrag: this.onDrag,
        onClick: function onClick(event) {
            _this.options.onClick(_this.isAdjusting);
        }
    }));
};
...
var _kinet = __webpack_require__(5);
